...
function agregarProducto(nombreProducto) {
  // Lista principal
  if (listaCarrito.children.length === 1 && listaCarrito.children[0].textContent.includes('No hay productos')) {
  ...
  // Lista offcanvas
  if (listaCarritoOffcanvas && listaCarritoOffcanvas.children.length === 1 && listaCarritoOffcanvas.children[0].textContent.includes('No hay productos')) {
    listaCarritoOffcanvas.innerHTML = '';
  }

  if (listaCarritoOffcanvas) {
  ...
  if (contadorCarrito) {
    contadorCarrito.textContent = listaCarritoOffcanvas ? listaCarritoOffcanvas.children.length : listaCarrito.children.length;
  }
}
